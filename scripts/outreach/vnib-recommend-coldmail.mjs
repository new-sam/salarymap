// KTC 9/8 VNIB TECH recommend — Event Marketing Specialist (Camon Social 앱 커뮤니티/이벤트, HCMC Q3 온사이트).
// JD: 2-4y Community/Event/Influencer Marketing · 영어 커뮤니케이션 필수 · CV 영문+포트폴리오 링크 · 뷰티 이벤트 우대.
// 9/8 풀 실측(이력서 보유·unsub·당일발송 제외): Marketing×HCMC×2y+×영어 54 ·
// 비MKT 이벤트시그널 확장 12 → 개발직군 제외(‘event-driven’ 오탐 실측)로 축소.
// ktc0907 패턴: 룰 캐스케이드 → 1인1통(당일 발송자 제외) → 공개/비공개 프레임.
//
//   node scripts/outreach/vnib-recommend-coldmail.mjs                          # dry-run
//   node scripts/outreach/vnib-recommend-coldmail.mjs --send [--group evtmkt] [--max N]
...
const JOBS = {
  VN1: 'd916df23-8a87-464f-a5ac-a1fe81f732f9', // VNIB TECH Event Marketing Specialist (HCMC Q3)
}
...
// JD의 자격 분야(Community/Event/Influencer/Engagement) 실경험 시그널 — 개발 문서의 event-driven류 오탐은 g2 직군 제한으로 차단.
const evtRe = /(event|sự kiện|su kien|community|cộng đồng|cong dong|influencer|\bkol\b|activation|brand ambassador|meetup|workshop|user engagement)/i
const beautyRe = /(mỹ phẩm|my pham|beauty|cosmetic|skincare)/i
...
// 캐스케이드 순서 = 배정 우선순위, 1인 1그룹. 공통 하드컷: HCMC(온사이트)·2y+(JD 최소)·영어(JD 필수).
const score = (p) => locA(p) + (evtRe.test(txt(p)) ? 2 : 0) + (y(p) >= 24 && y(p) <= 48 ? 1 : 0) + (beautyRe.test(txt(p)) ? 1 : 0)
const GROUPS = [
  {
    gkey: 'evtmkt', brand: 'vnib', jobKey: 'VN1', camp: 'vnib-recommend1-evtmkt',
    label: { vi: 'Event Marketing Specialist', ko: '이벤트 마케팅' },
    pick: (p) => (hasAny(p, ['Marketing']) && inHcmc(p) && y(p) >= 24 && enOk(p) ? score(p) : null),
  },
  {
    // 확장: 타직군이지만 이벤트/커뮤니티 실경험 시그널 보유(개발직군 제외 — event-driven 오탐 차단).
    gkey: 'evtmkt2', brand: 'vnib', jobKey: 'VN1', camp: 'vnib-recommend1-evtmkt2',
    label: { vi: 'Event Marketing Specialist', ko: '이벤트 마케팅(확장)' },
    pick: (p) => (!hasAny(p, ['Marketing']) && !hasAny(p, DEVISH) && inHcmc(p) && y(p) >= 24
      && evtRe.test(txt(p)) && enOk(p) ? score(p) : null),
  },
]

// ── 카피(vi 실발송) — ktc0907 정직 프레임(공개/비공개) ──
const BRANDS = {
  vnib: {
    company: 'VNIB TECH', initial: 'V', meta: 'Onsite · TP.HCM (Quận 3) · 2–4 năm',
    intro: '<b>VNIB TECH</b> — công ty đứng sau app cộng đồng <b>Camon Social</b> — đang tuyển <b>Event Marketing Specialist</b> qua FYI, làm việc tại TP.HCM (tòa nhà Toong, Quận 3). Công việc: xây dựng và phát triển cộng đồng, thu hút user mới, tổ chức Meetup/Event offline, hợp tác với KOL/Influencer, venue và đối tác. Yêu cầu 2–4 năm kinh nghiệm Community Management / Event Marketing / Influencer Marketing và giao tiếp tiếng Anh tốt; ưu tiên kinh nghiệm event ngành mỹ phẩm/beauty (không bắt buộc). Lưu ý: cần <b>CV bằng tiếng Anh</b> (kèm link portfolio trong CV).',
  },
}
const SUBJECT = {
...
async function main() {
  ...
  for (const { p, g, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/vnib-recommend-coldmail',
