// GasDNA(가스디엔에이, 가스검지기·가스분석기 제조) 한국행 펌웨어·회로 엔지니어 추천 콜드메일 — rothea/bada 표준.
// 배경: 8/26 웍스피어 신청(8/7 접수·나인하이어 이슈로 지연 감지). 한국 연구소 근무(E-7), 45-48M,
//   ITS와 달리 한국어 요건 없음 → 풀 소싱 가능. 대상은 룰 기반 스캔 확정 data/gasdna-candidates.json 16명
//   — 컷: E-7(경력 1y+ or 석사) × 실질 임베디드/전자 결. A(5) 펌웨어 정합 / B(5) 인접 강 / C(6) 인접.
// ⚠️ 카피가 "이번 주 담당자에게 명단 전달" 약속 — 발송 후 GasDNA(준현 라인)에 추천 명단 실제 공유할 것.
//
//   node scripts/outreach/gasdna-recommend-coldmail.mjs                 # dry-run
//   node scripts/outreach/gasdna-recommend-coldmail.mjs --send [--max N]
import { readFileSync } from 'node:fs'
import { Resend } from 'resend'
import { sb, env, fetchAll } from './lib.mjs'
import { makeToken } from '../../lib/campaignToken.js'

const JOB_ID = '44e5a175-df34-4128-8cb2-6d20d716d664' // GasDNA — Kỹ sư Firmware & Thiết kế Mạch điện tử (45-48M, Korea)
const ROLE_VI = 'Kỹ sư Firmware & Thiết kế Mạch điện tử (làm việc tại Hàn Quốc)'
const CANDIDATES = JSON.parse(readFileSync(new URL('../../data/gasdna-candidates.json', import.meta.url), 'utf8'))
...
const campaignOf = (frame) => `gasdna-recommend1-${frame}`

// ── 카피(vi 실발송) — kyndof 표준 정직 프레임(공개/비공개) ──
const COPY = {
  subject: {
    public: `[FYI] Bạn được chọn vào danh sách đề cử gửi GasDNA — ${ROLE_VI}`,
    private: `[FYI] Bạn được chọn vào danh sách đề cử — ${ROLE_VI} tại GasDNA`,
  },
  ...
  hook: 'Đội ngũ FYI đã xem xét toàn bộ hồ sơ đã đăng ký và <b>chọn bạn vào danh sách đề cử</b> cho vị trí dưới đây — kinh nghiệm embedded/firmware và nền tảng kỹ thuật điện tử của bạn phù hợp với yêu cầu của vị trí này.',
  benefit: {
    public: '<b>Trong tuần này</b>, FYI sẽ gửi danh sách đề cử trực tiếp cho người phụ trách tuyển dụng của GasDNA. Hồ sơ của bạn đang ở chế độ công khai nên sẽ được gửi kèm danh sách. Nếu bạn ứng tuyển ngay, CV của bạn sẽ được <b>ưu tiên xem xét</b> cùng lời giới thiệu từ FYI.',
    private: '<b>Trong tuần này</b>, FYI sẽ gửi danh sách đề cử trực tiếp cho người phụ trách tuyển dụng của GasDNA. Hồ sơ của bạn đang ở chế độ riêng tư — nếu bạn ứng tuyển ngay, CV của bạn sẽ được gửi kèm lời giới thiệu từ FYI và được <b>ưu tiên xem xét</b>.',
  },
...
function jobCard(job) {
  const logo = job.logo_url
    ...
    : `<div style="width:44px;height:44px;border-radius:10px;background:#fff0e6;color:#ff6000;font-weight:800;font-size:16px;text-align:center;line-height:44px">G</div>`
  const meta = [salaryLine(job), 'Hàn Quốc (E-7)'].filter(Boolean).map(esc).join(' · ')
  return `<table width="100%" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #eee5da;border-radius:14px;margin-bottom:8px"><tr>
    ...
    <td style="padding:14px 14px 14px 12px;vertical-align:middle">
      <div style="font-size:12px;color:#8a8073;margin-bottom:3px">GasDNA</div>
...
- ${job.title.trim()} (GasDNA) — ${[salaryLine(job), 'Hàn Quốc (E-7)'].filter(Boolean).join(' · ')} — ${SITE}/ktc/jobs/${job.id}
...
async function main() {
  ...
  for (const { p, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/gasdna-recommend-coldmail',
