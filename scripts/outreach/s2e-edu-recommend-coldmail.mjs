// KTC 9/9 s2e 재소싱 recommend — 파트너 개발 CTV(R148), 교육업계 영업 네트워크 앵글.
// 배경: 1차 추천(9/4~9/8 캠페인 유입) 전원 반려 — "학원/학교 쪽 영업/네트워크 가진 사람 희망"(호현 9/9).
// 새 룰: 교육업계 실경험 시그널(tuyển sinh·어학원·유학원·edtech 등) × 영업·상담 시그널 하드컷.
// 9/9 실측 85명(신규 79 / 기수신 재발송 6, 기지원 14 제외). 공고 우대 문구도 동일 앵글로 수정 반영됨.
// 재발송 표준: 기지원만 제외(조건 변경 시 기수신 재발송 허용 — dn0909 전례). 접두어 s2e-edu- (구 s2e-recommend\d- 템플릿 충돌 회피).
//
//   node scripts/outreach/s2e-edu-recommend-coldmail.mjs                          # dry-run
//   node scripts/outreach/s2e-edu-recommend-coldmail.mjs --send [--max N]
...
const JOBS = {
  S2E: '271c6595-052e-437d-ad4c-430bd0b50595', // s2e R148 Cộng tác viên phát triển đối tác (Remote)
}

// ── 대상 선정: 교육업계 시그널 × 영업 시그널 (성조 제거 정규화 후 매칭 — 풀 실측과 동일) ──
const norm = (s) => String(s || '').normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/đ/gi, 'd').toLowerCase()
const txt = (p) => norm(JSON.stringify(p.skills || '') + ' ' + String(p.resume_summary || '') + ' ' + JSON.stringify(p.experiences || '') + ' ' + String(p.position || ''))
// 학원 영업의 실제 직무 어휘: tuyen sinh(원생 모집)·어학원/센터·유학원·edtech·교육영업 등
const eduRe = /(tuyen sinh|trung tam (tieng|anh ngu|ngoai ngu|han ngu)|language center|english center|edtech|du hoc|giao duc|education (sales|consult|counsel)|academy|hoc vien|dao tao.{0,20}(kinh doanh|tu van)|admission)/
const salesRe = /(sales|kinh doanh|tu van|business development|\bbd\b|b2b|telesale|account)/
const sportRe = /(the thao|sport|gym|fitness|bong da|football)/
const roles = (p) => new Set([p.position, ...(p.desired_roles || [])].filter(Boolean))
const hasSalesRole = (p) => [...roles(p)].some((r) => ['Sales', 'Business Development', 'Marketing'].includes(r))

// Remote 공고라 지역 컷 없음. tuyen sinh(원생 모집 실무)이 최상위 시그널, 스포츠 교집합 가산.
const score = (p) => { const t = txt(p); return (/tuyen sinh/.test(t) ? 2 : 0) + (sportRe.test(t) ? 1 : 0) + (hasSalesRole(p) ? 1 : 0) }
const GROUPS = [
  {
    gkey: 'edu', brand: 's2e', jobKey: 'S2E', camp: 's2e-edu-recommend2',
    label: { vi: 'Cộng tác viên phát triển đối tác', ko: '파트너 개발(교육 네트워크)' },
    pick: (p) => { const t = txt(p); return eduRe.test(t) && (salesRe.test(t) || hasSalesRole(p)) ? score(p) : null },
  },
]

// ── 카피(vi 실발송) — 교육 네트워크 앵글의 정직 프레임(공개/비공개) ──
const BRANDS = {
  s2e: {
    company: 's2e', initial: 'S', meta: 'Remote · Freelance/CTV theo dự án POC · 7–9 triệu',
    intro: '<b>s2e</b> — công ty Hàn Quốc trong lĩnh vực thể thao – giáo dục, đang mở rộng thí điểm (POC) ra thị trường Việt Nam — đang tuyển <b>Cộng tác viên phát triển đối tác</b> qua FYI: tìm kiếm và kết nối các học viện thể thao, trung tâm đào tạo, trường học tại địa phương. Làm việc từ xa, hình thức cộng tác viên theo dự án. Lần này công ty <b>đặc biệt ưu tiên ứng viên có kinh nghiệm sales/tư vấn tuyển sinh hoặc mạng lưới quan hệ với học viện, trung tâm, trường học</b> — kinh nghiệm trong lĩnh vực giáo dục của bạn chính là điều họ đang tìm.',
  },
}
const SUBJECT = {
...
const HOOK = 'Đội ngũ FYI đã xem xét toàn bộ hồ sơ đã đăng ký và <b>chọn bạn vào danh sách đề cử</b> cho vị trí dưới đây — hồ sơ của bạn có kinh nghiệm/mạng lưới trong lĩnh vực giáo dục phù hợp nhất với yêu cầu lần này.'
const BENEFIT = {
...
async function main() {
  ...
  // 재발송 표준: 기지원(1차 추천 반려자 포함)만 제외 — 기수신자는 새 앵글로 재발송 허용
  const applied = (jobKey, uid) => (appliedByJob[JOBS[jobKey]] || new Set()).has(uid)
  ...
  for (const p of pool) {
    ...
    for (const g of GROUPS) {
      const s = g.pick(p)
      if (s == null) continue
      if (applied(g.jobKey, p.id)) continue
  ...
  if (!doSend) {
    for (const k of keys) {
      ...
      console.log(`\n── ${k} 상위 8 ──`)
      for (const { p, s, frame } of rows.slice(0, 8))
  ...
  for (const { p, g, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/s2e-edu-recommend-coldmail',
