// JIC Global(JIC3302)·Moen(ME1705) 지원미달 공고 2건 추천 콜드메일 — 1인 1통 배정 + 공개/비공개 프레임.
// 배경: 8/18 어드민 트래커 기준 JIC3302 D+26 지원 0/10, ME1705 D+6 지원 3/10.
//   - JIC3302 ONLINE MD / PERFORMANCE MARKETER — 서울 성수 온사이트, 한국어 필수, 이커머스·퍼포마케팅 3y+.
//     ⚠️ is_active=false 상태(공고목록·원탭 랜딩 모두 비노출) — 발송 전 반드시 활성화할 것. 스크립트가 비활성 공고는 발송 거부.
//   - ME1705 AI-Powered Designer (Trainee) — HCM·하노이·다낭, 경력무관, AI 이미지 툴(개인 프로젝트 인정).
//
// 대상 = 키워드·어학 게이트 × gpt-4o-mini 1~5 채점(--filter, 캐시 data/jic-moen-coldmail-scores.json, 키 "코드:userId").
//   jic1: 마케팅/이커머스 키워드 × 한국어(korean_cert 유효 또는 TOPIK 언급) × 경력 2.5y+ → 3점+(풀 얇아 완화, ops1 전례)
//   moen1: 디자인 키워드 × 경력 3y 이하(인턴 급여라 시니어 제외) → 4점+
// 1인 1통: 두 그룹 다 통과 시 고점 그룹, 동점이면 jic1(모수 얇은 쪽 우선 — kyndof 규칙).
// 프레임: is_resume_public 기준 캠페인 분리(zest 표준) — {jic,moen}-recommend1-{public,private}.
//   카피는 양쪽 다 "FYI가 검토해 추천 명단 선정" 정직 프레임(no-talent-showcasing — "담당자가 봤다"는 안 씀).
// ⚠️ 카피가 "이번 주 담당자에게 명단 전달"을 약속 — 발송 후 JIC·Moen 측에 추천 명단 실제 공유할 것.
//
//   node scripts/outreach/jic-moen-recommend-coldmail.mjs --filter                 # LLM 채점 캐시 생성(멱등)
//   node scripts/outreach/jic-moen-recommend-coldmail.mjs                          # dry-run: 그룹별 집계
//   node scripts/outreach/jic-moen-recommend-coldmail.mjs --test wsj@likelion.net  # 검수용 한국어 4통(그룹×프레임), 이벤트 기록 없음
//   node scripts/outreach/jic-moen-recommend-coldmail.mjs --send [--max N] [--group jic1]
...
// ── 후보 텍스트/게이트 유틸(kyndof 채점과 동일 방식) ──
...
// 한국어: korean_cert 직접기입값(무의미값 제외) 또는 텍스트에 TOPIK 명시. 'korean' 텍스트 검색은 오탐(welford 교훈).
const krOk = (p) => {
  const c = String(p.korean_cert || '').trim()
  if (c && !/^(none|beginner|novice)$/i.test(c)) return true
  return /topik/i.test(candText(p))
}
const MKT_RE = /marketing|growth|\bseo\b|\bsem\b|performance|e-?commerce|online md|merchandis|smart ?store|shopee|lazada|tiki\b|tiktok shop|\broas\b|quảng cáo|truyền thông/
const DSG_RE = /design|photoshop|illustrator|figma|graphic|đồ họa|thiết kế|midjourney|firefly|stable diffusion|ui\/ux|\bux\b|\bui\b/

// ── 그룹 정의: 공고 코드·게이트·채점 rubric·카피(회사 소개는 JD 원문에서만, 수치 없음) ──
const GROUPS = [
  {
    key: 'jic1', code: 'JIC3302', min: 3,
    label: { vi: 'Online MD / Performance Marketer', ko: '온라인 MD / 퍼포먼스 마케터' },
    company: 'JIC Global',
    locationDisplay: 'Seoul (Seongsu-gu), Hàn Quốc',
    // JD는 3y+지만 한국어×마케팅 풀이 극히 얇아 1.5y+까지 채점(경력 미달은 rubric이 감점) — 발송 임계는 min으로 제어
    gate: (p) => (p.yoe_months ?? 0) >= 18 && krOk(p) &&
      (MKT_RE.test(candText(p)) || [...posOf(p)].some((x) => ['Marketing', 'Merchandiser'].includes(x))),
    rubric: `"Online MD / Performance Marketer" 포지션 적합도를 1~5로 채점하라.
공고: 한국 패션·스니커 브랜드 EPT의 온라인 커머스 운영+퍼포먼스 마케팅. 근무지 서울 성수(한국 온사이트). 필수: 한국어 커뮤니케이션 능숙, 관련 경력 3년+, 이커머스 운영 이해, 퍼포먼스 마케팅 실무(ROAS·전환 분석), 데이터 기반 의사결정. 우대: Smart Store·MUSINSA·29CM 등 커머스 플랫폼 운영, GA4·광고플랫폼 데이터 분석.
5=이커머스 운영 또는 퍼포먼스 마케팅 3년+ 실무이고 한국어 능숙(TOPIK 4+ 또는 상응), 4=마케팅/이커머스 실무 경력이 뚜렷하고 한국어 가능하나 한쪽 증거가 다소 약함, 3=마케팅 계열이고 한국어 있으나 퍼포먼스/이커머스 직접 증거 약함, 2=세일즈·CS 등 인접 직군이거나 한국어 근거 희박, 1=무관.`,
  },
  {
    // 3점=디자이너인데 AI툴/F&B 증거만 부족 — AI툴은 개인 프로젝트 인정(JD)이고 CV에 잘 안 적혀 3점까지 포함
    key: 'moen1', code: 'ME1705', min: 3,
    label: { vi: 'Brand · AI-Powered Designer (Trainee)', ko: '브랜드 · AI 디자이너 (인턴/트레이니)' },
    company: 'Moen',
    locationDisplay: 'Hồ Chí Minh · Hà Nội · Đà Nẵng',
    gate: (p) => (p.yoe_months ?? 0) <= 36 &&
      (DSG_RE.test(candText(p)) || [...posOf(p)].some((x) => ['Design'].includes(x))),
    rubric: `"Brand · AI-Powered Designer (인턴/트레이니)" 포지션 적합도를 1~5로 채점하라.
공고: 한국 F&B 브랜드 Moen의 SNS·커머스(PDP·인스타그램) 비주얼 콘텐츠 디자인. 필수: 디자인 전공(재학 포함) 또는 포트폴리오로 증명, Photoshop/Illustrator 기본기, AI 이미지 생성 툴(Midjourney·Firefly 등) 실사용 경험(개인/사이드 프로젝트 인정), 레퍼런스의 tone&manner 분석→프롬프트화. 경력 무관·신입 환영(인턴 급여 수준). 우대: Beauty/F&B/커머스 비주얼 제작, AI 합성 이미지의 어색함 감지·수정, 프롬프트 엔지니어링.
...
const COPY = {
  ...
  intro: {
    jic1: {
      vi: '<b>JIC Global</b> đang tuyển vị trí <b>Online MD / Performance Marketer</b> cho <b>EPT (East Pacific Trade)</b> — thương hiệu thời trang & sneaker Hàn Quốc theo phong cách minimal·streetwear. Vị trí làm việc trực tiếp tại <b>Seoul (Seongsu-gu), Hàn Quốc</b>, yêu cầu <b>giao tiếp tốt bằng tiếng Hàn</b> và tối thiểu 3 năm kinh nghiệm vận hành e-commerce / performance marketing.',
      ko: '<b>JIC Global</b>이 미니멀·스트리트웨어 기반 한국 패션·스니커 브랜드 <b>EPT (East Pacific Trade)</b>의 <b>온라인 MD / 퍼포먼스 마케터</b>를 채용 중입니다. <b>서울 성수동 온사이트</b> 근무이며, <b>한국어 커뮤니케이션 능숙</b>과 이커머스 운영/퍼포먼스 마케팅 경력 3년 이상이 필요합니다.',
    },
    moen1: {
...
async function main() {
  ...
  const jobs = await fetchAll(() => sb.from('jobs')
    ...
    .in('source_id', GROUPS.map((g) => g.code)).order('source_id'))
  const jobByCode = Object.fromEntries(jobs.map((j) => [j.source_id, j]))
  for (const g of GROUPS) {
    const j = jobByCode[g.code]
    if (!j) { console.error(`공고 없음: ${g.code}`); process.exit(1) }
...
요약: ${JSON.stringify(p.resume_summary || {}).slice(0, 500)}`
    ...
    console.log(`채점 완료(신규 ${n}) → data/jic-moen-coldmail-scores.json`)
    return
  }
  ...
  // ── 1인 1통 배정: 임계 통과 그룹 중 고점, 동점이면 GROUPS 정의 순(jic1 우선 — 모수 얇은 쪽) ──
  ...
  if (!doSend) {
    for (const { p, g, score, frame } of list.slice(0, 40))
      console.log(`  [${score}·${frame}] ${g.key} · ${p.full_name} <${p.email}> · ${Math.round((p.yoe_months || 0) / 12 * 10) / 10}y · 한국어:${p.korean_cert || '-'} · ${(p.headline || p.position || '').slice(0, 44)}`)
  ...
  for (const { p, g, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/jic-moen-recommend-coldmail',
