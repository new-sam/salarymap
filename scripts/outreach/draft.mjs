// 회사별 개인화 콜드메일 초안 생성 (OpenAI). 플레인텍스트, 한국어 존댓말, B2B.
import { openai } from './lib.mjs'
...
// 수신거부 문구·서명은 발송 시점에 lib.composeHtml 이 붙인다 (내 Gmail 설정 서명 사용).

const SYSTEM = `당신은 베트남에 진출한 한국기업을 상대하는 B2B 세일즈 담당자입니다.
...
- 읽기 쉽게 줄바꿈: 한 문장(또는 짧은 의미 단위)마다 줄바꿈(\\n)을 넣고, 인사·본론·제안·맺음 블록 사이엔 빈 줄(\\n\\n). 여러 문장을 한 줄에 붙이지 말 것. 문장은 짧게 끊을 것.
- 인사 다음, 그 회사의 사업내용·업종을 구체적으로 언급해 "당신 회사를 알고 보냈다"는 느낌을 줄 것. 과장/아부 금지.
- 그 회사가 베트남에서 필요로 할 만한 직무(사업내용에서 유추 — 개발·웹·디자인·마케팅·무역/영업·사무 등)와 자연스럽게 연결할 것. 특정 직무를 단정하지 말고 "귀사에 맞는 직무의 베트남 인재"처럼 유연하게.
- 전체 4~6문장, 짧게. 한 가지 CTA만.
- 스팸처럼 보이는 표현(무료!!!, 대박, 지금 당장 등) 금지. 담백하고 신뢰감 있게.
- 본문은 반드시 아래처럼 "세 줄"로 시작할 것 (각 문장마다 줄바꿈):
안녕하세요, {담당자} 대표님.
Likelion(멋쟁이사자처럼)의 위승주라고 합니다.
처음 인사드립니다.
  (담당자 이름을 모르면 첫 줄을 "안녕하세요, 담당자님." 으로. 반드시 세 줄로 나눌 것.)
- 본문은 인사 → 회사 언급 → 제안 → CTA 순.
- 끝맺음은 반드시 "감사합니다." 줄바꿈 후 "위승주 드림" 으로 끝낼 것.
...
export async function generateDraft(lead) {
  const ctx = {
    회사명: lead.company_ko || lead.company_en,
    담당자: lead.contact_ko || lead.contact_en || null,
