// 코스모스소프트(K22) Full-stack Junior 한국 근무 recommend — 9/11 Mia FYI 등록 당일 발송.
// JD: 보험사 SI/SM(자사 CRM 기반), Java/Spring + HTML/JS/React + SQL/DBMS + 형상관리 + 클라우드 이해.
//     한국어 커뮤니케이션 "필수" · 영문 CV 제출 · 근무지 한국(서울 종로) · 연 23~25M KRW(공고 표기).
// 9/11 풀 실측(기준 3,202 · unsub/likelion/중복 제외): FS/BE/FE 직군 × 한국어 인증 = 16명뿐(개발 922 중).
//   core = +Java/Spring 근거 8명(전원 ≤3y) / ext = 한국어 개발자 나머지 중 ≤3y (Junior 포지션이라 3y+ 미스매치 컷).
// ktc0907b 정직 프레임 · 1인1통(당일 recommend 기수신 제외) · 공개/비공개 프레임.
//
//   node scripts/outreach/cosmos0911-recommend-coldmail.mjs                       # dry-run
//   node scripts/outreach/cosmos0911-recommend-coldmail.mjs --send [--group core] [--max N] [--gap-hours N]
...
// 가점: JD 스택 커버리지 + 영문 CV 대비 영어 인증
const score = (p) => (javaSig(p) ? 3 : 0) + (feSig(p) ? 1 : 0) + (sqlSig(p) ? 1 : 0) + (p.english_cert ? 1 : 0)

// 하드게이트: 개발직군 × 한국어(JD 필수) × ≤3y(Junior). core=Java/Spring 근거, ext=나머지.
const GROUPS = [
  {
    gkey: 'core', camp: 'cosmos0911-recommend1-core',
    label: { vi: 'Full-stack Developer (Junior)', ko: '풀스택 주니어(Java/Spring 근거)' },
    pick: (p) => (devRole(p) && koSig(p) && y(p) <= 36 && javaSig(p) ? score(p) : null),
  },
  {
    gkey: 'ext', camp: 'cosmos0911-recommend1-ext',
    label: { vi: 'Full-stack Developer (Junior)', ko: '풀스택 주니어(확장: 한국어 개발자)' },
    pick: (p) => (devRole(p) && koSig(p) && y(p) <= 36 ? score(p) : null),
  },
]

// ── 카피(vi 실발송) — 조건(한국 근무·한국어·영문 CV·스택) 전부 명시해 자기선별 유도 ──
const COMPANY = 'Cosmos Soft'
const INITIAL = 'C'
const META_VI = 'Onsite · Seoul, Hàn Quốc · 23–25 triệu KRW/năm'
const INTRO = '<b>Cosmos Soft</b> — công ty IT Hàn Quốc chuyên SI/SM, xây dựng hệ thống TM, GA, CS, QA cho các công ty bảo hiểm dựa trên giải pháp CRM riêng (cosmossoft.co.kr) — đang tuyển <b>Full-stack Developer (Junior)</b> <b>làm việc tại Hàn Quốc</b> qua FYI. Công việc: tham gia dự án SI xây dựng &amp; nâng cấp hệ thống cho công ty bảo hiểm, thiết kế cơ bản &amp; implementation business logic, giám sát hệ thống, kiểm tra dữ liệu &amp; unit test. Yêu cầu: <b>Java, Spring Framework</b>; HTML/JavaScript/<b>React</b>; thiết kế DB &amp; <b>SQL</b> (Oracle, MySQL); công cụ quản lý version; hiểu biết cloud (AWS, Azure); <b>giao tiếp được bằng tiếng Hàn</b>. Lương 23–25 triệu KRW/năm. <b>Nộp CV bằng tiếng Anh.</b>'
const SUBJECT = {
...
const HOOK = 'Đội ngũ FYI đã xem xét toàn bộ hồ sơ đã đăng ký và <b>chọn bạn vào danh sách đề cử</b> cho vị trí dưới đây — hồ sơ của bạn (tiếng Hàn + nền tảng phát triển web) phù hợp với yêu cầu của vị trí này.'
const BENEFIT = {
...
async function main() {
  ...
  for (const { p, g, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/cosmos0911-recommend-coldmail',
