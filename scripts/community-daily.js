...
//
// Usage:
//   node scripts/community-daily.js --tick [--dry] [--force]   ← scheduled trickle (1 post sometimes + comment drip)
//   node scripts/community-daily.js [--count N] [--dry]         ← one-off batch of N posts
//   --dry     generate + print, insert nothing
//   --force   (tick) always post once this run, ignore active-hour gate
//   --count   (batch) how many posts this run (default 6)
require('dotenv').config({ path: '.env.local' })
const { createClient } = require('@supabase/supabase-js')
const OpenAI = require('openai')
const { runDailySeed, runTick } = require('../lib/communityContent')
