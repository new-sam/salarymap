...
// --all: 공개 이력서 전체 재파싱(요약 양식 변경 시). updated_at은 안 건드림(지표 착시 방지).
...
const ids = await findPublicUnparsed(limit, all)
console.log(`파싱 대상(공개${all ? '·전체' : '·필드 빈'}): ${ids.length}명`)
if (dry) { console.log(ids); process.exit(0) }

let ok = 0, fail = 0
for (const id of ids) {
  try {
    const u = await parseResumeForUser(id, { touchUpdatedAt: !all })
    ok++
