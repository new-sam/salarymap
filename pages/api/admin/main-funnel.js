...
export default async function handler(req, res) {
  ...
  try {
    const [allUsers, appsRaw] = await Promise.all([
      ...
      fetchAll(
        supabase.from('job_applications')
          .select('id, created_at, status, application_source, applicant_email, user_id, jobs(title, company)')
    ...
    const appDaily = {}       // date → { count, users:Set }
    const appSourceMap = {}
    const appStatusMap = {}
    const jobMap = {}         // company·title → { company, title, count, accepted }
    for (const a of apps) {
      const d = toVN(a.created_at)
      if (!appDaily[d]) appDaily[d] = { count: 0, users: new Set() }
      appDaily[d].count++
      ...
      if (!jobMap[jk]) jobMap[jk] = { company: a.jobs?.company || '—', title: a.jobs?.title || '—', count: 0, accepted: 0 }
      jobMap[jk].count++
    ...
    res.status(200).json({
      ...
      applications: {
        ...
        daily: Object.fromEntries(Object.entries(appDaily).map(([d, v]) => [d, { count: v.count, users: v.users.size }])),
