...
// 회원 대상 콜드메일 수신거부 — KTC판(/api/ktc/unsub, lead 해시)과 같은 설계를
// 회원(user_id 토큰)용으로 옮긴 것. 별도 테이블 없이 events 에 coldmail_unsub 을 남기고,
// 발송 스크립트가 user_id 기준으로 대상에서 뺀다(어느 회원 캠페인이든 공용).
  ...
  if (parsed) {
    try {
      await supabase.from('events').insert([{
        ...
        page: '/api/coldmail/unsub',
        user_id: parsed.userId,
        meta: { campaign: parsed.campaign, via: req.method === 'POST' ? 'oneclick' : 'link' },
      }])
    } catch (e) {
      console.error('coldmail/unsub log failed:', e.message)
    }
  }
