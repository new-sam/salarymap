...
JSON 만 출력하세요:
{
  ...
  "title_ko": "이 후보의 직무를 한국어로 짧게. 반드시 한국 채용 시장에서 실제로 쓰는 표기로 씁니다 — 외래어 직함은 그대로 한글 표기합니다 (프로덕트 매니저, 비즈니스 애널리스트, 데이터 애널리스트, 백엔드 개발자, QA 엔지니어, UX 디자이너). 직역 금지: 제품관리자(✗)·사업분석가(✗)·자료분석가(✗). 이력서에 근거가 있으면 전문 분야가 드러나게 씁니다 (예: 결제 시스템 백엔드 개발자, 이커머스 프론트엔드 개발자). 분야 근거가 없으면 직무만 씁니다 — 지어내지 마세요. 회사명·사람 이름·연차는 넣지 마세요.",
  "score_breakdown": {
...
JSON 만 출력하세요:
{"picks":[{"i":후보번호,"fit":0-100,"why":"이 분을 왜 추천하는지 한국어 한 문장 (아래 쓰기 규칙을 지킬 것)."}]}

why 는 한 문장입니다. 카드에 기술 칩과 경력사항이 이미 붙어 있어서, 거기 적힌 것을
...
function card(p, v, c) {
  ...
  return {
    ...
    skillsMore: Math.max(0, skills.length - 8),
