...
export default function CompanyDetailPanel({
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 14 }}>
          {[
            ...
            <div key={s.label} style={{
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: s.accent ? 'var(--sm-accent)' : '#fff' }}>{s.value}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 6 }}>
          {boxes.map(s => (
            <div key={s.label} style={{
              ...
              <div style={{ fontSize: 22, fontWeight: 800, color: s.hl ? 'var(--sm-accent)' : '#1a1a1a', letterSpacing: '-0.02em' }}>{s.value}</div>
              <div style={{ fontSize: isMobile ? 12 : 10, fontWeight: 600, color: s.hl ? 'var(--sm-accent)' : '#888', marginTop: 4 }}>{s.label}</div>
            </div>
          ))}
        </div>
        {summaryData && (
      ...
      <div style={{ margin: '14px 0 18px' }}>
        ...
        <div style={{ position: 'relative', height: 6, background: '#f0f0f0', borderRadius: 3 }}>
          ...
          <div style={{
            ...
            background: 'linear-gradient(90deg, #ffcab3, var(--sm-accent))',
            width: `${barWidth}%`,
          }} />
          {/* Median marker */}
          <div style={{
            position: 'absolute', top: -4, width: 14, height: 14, borderRadius: '50%',
            background: 'var(--sm-accent)', border: '2px solid #fff',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 9, color: '#aaa' }}>
          ...
          <span style={{ color: 'var(--sm-accent)', fontWeight: 600 }}>{t('detail.medianLabel', { value: fmtShort(summaryData.median) })}</span>
  ...
  const renderFeedRow = (row, i) => {
    ...
    const bd = isYou ? '1.5px solid var(--sm-accent)' : row.mostSimilar ? '1px solid #86efac' : 'none'
    return (
      <div key={i} style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {isYou && <span style={{ fontSize: isMobile ? 11 : 9, fontWeight: 700, color: 'var(--sm-accent)', background: 'rgba(255,68,0,0.08)', padding: '1px 6px', borderRadius: 3 }}>YOU</span>}
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: isYou ? 'var(--sm-accent)' : '#1a1a1a' }}>{fmt(row.salary)}</div>
  ...
  const renderGate = () => (
    <div style={{
      ...
      <button onClick={() => {
        ...
        background: 'var(--sm-accent)', border: 'none', borderRadius: 12,
              ...
              {!loading && detail && isSubmitted && !isMyCompany && (
                <>
                  ...
                  <a href="/jobs" style={{
                    ...
                    background: '#1a0d07', border: '1px solid rgba(255,68,0,0.25)',
                    borderRadius: 10, textAlign: 'center', textDecoration: 'none',
                    fontSize: 13, fontWeight: 700, color: 'var(--sm-accent)',
                  }}>
