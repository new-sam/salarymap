...
export const OWNERS = {
  wsj: { sender: 'wsj@likelion.net', name: '위승주', sigReplace: [/AI PM Intern/g, 'AI Product Manager'], imageUrl: '/outreach/wsj-visual.png' },
  younghun: { sender: 'younghun@likelion.net', name: '남영훈', sigReplace: null, imageUrl: '/outreach/younghun-visual.png' },
}
...
export async function sendOutreach(ownerKey, { to, subject, body, leadId }) {
  ...
  const raw = Buffer.from([
    ...
    '',
    b64(html(body, sig, `${base}/api/o/${leadId}`, owner.imageUrl ? `${base}${owner.imageUrl}` : null)),
  ].join('\r\n')).toString('base64url')
