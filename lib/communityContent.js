...
// view/like distribution copied from insert-bulk-community.js so posts blend in
function postStats() {
  const r = Math.random()
  const view = r < 0.1 ? rint(800, 3000) : r < 0.8 ? rint(80, 600) : rint(12, 90)
  const ratio = 0.004 + Math.pow(Math.random(), 2) * 0.045
...
- bình luận tự nhiên 1-3 câu, nhiều góc nhìn khác nhau: đồng cảm, khuyên thẳng, kể trải nghiệm riêng, phản biện, hỏi lại.
...
- khoảng một nửa để is_anonymous = true, nửa còn lại false.
...
Mỗi post 3-7 comments, số lượng KHÁC NHAU rõ rệt giữa các post (có post chỉ 3, có post 6-7).`
...
async function runDailySeed({ supabase, openai, count = 6, dry = false, log = () => {} }) {
  ...
  for (const p of posts) {
    ...
    const comments = Array.isArray(p.comments) ? p.comments.slice(0, 7) : []

    if (dry) {
      log(`\n[${category}] ${p.title}  — by ${p.is_anonymous ? maskName(p.author_name) : p.author_name}`)
      log(p.content)
      comments.forEach((c) => log(`   └ ${c.is_anonymous ? maskName(c.author_name) : c.author_name}: ${c.content}`))
      continue
    }
...
- casual, viết thường, 1-3 câu, tự nhiên và hợp ngữ cảnh bài đăng: đồng cảm / khuyên thẳng / kể trải nghiệm riêng / phản biện / hỏi lại.
...
- khoảng 50% để is_anonymous = true.
...
// stable per-post target comment count (3..7), so a post fills up then stops
const targetComments = (id) => 3 + (hashCode(id) % 5)
...
async function runTick({ supabase, openai, log = () => {}, dry = false, postProb = 0.35, force = false }) {
  ...
  if (force || Math.random() < postProb) {
    ...
    for (const p of generated) {
      const category = CATEGORIES.includes(p.category) ? p.category : 'daily'
      if (dry) {
        log(`[would post] [${category}] ${p.title} — by ${p.is_anonymous ? maskName(p.author_name) : p.author_name}`)
        posted++
        continue
      }
  ...
  const { data: recent } = await supabase
    .from('community_posts')
    .select('id, title, content, comment_count, view_count, created_at')
    .eq('user_id', sys.id)
  ...
  for (const p of eligible.slice(0, 2)) {
    ...
    if (dry) {
      log(`[would comment] "${p.title}" ← ${c.is_anonymous ? maskName(c.author_name) : c.author_name}: ${c.content}`)
      commented++
      continue
    }
    ...
    // bump comment_count and nudge views so the post looks alive
    await supabase
      .from('community_posts')
      .update({ comment_count: (p.comment_count || 0) + 1, view_count: (p.view_count || 0) + rint(6, 25) })
      .eq('id', p.id)
    commented++
...
module.exports = { runDailySeed, runTick, SYSTEM_PROMPT }
