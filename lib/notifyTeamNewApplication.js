...
`<div style="font-family:'Pretendard','Segoe UI',Arial,sans-serif;color:#111;max-width:560px;margin:0 auto;padding:8px 0">
  ...
  <p style="font-size:12px;color:#9ca3af;margin-top:24px">Đây là email tự động. Vui lòng không trả lời trực tiếp.</p>
    ...
    // 7) 순차 발송 — Resend 는 2 req/sec 제한이 있어 병렬로 여러 명에게 쏘면
    //    일부가 rate-limited (429) 로 떨어진다. 팀 3명 이상일 때 조용히 누락되던 버그.
    //    수신자 간 이메일 노출 방지 위해 개별 send + 개별 log 기록.
    ...
    for (let i = 0; i < toEmails.length; i++) {
      ...
      results.push({ to, status, err });
      // Resend rate limit (2 req/sec) 회피 — 마지막 수신자 뒤에는 대기 불필요
      if (i < toEmails.length - 1) await sleep(600);
    }
