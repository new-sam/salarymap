 ...
 * @param {string} opts.category  push_tokens.prefs 키(like|comment|application|company_follow|daily_hot|marketing)
  ...
  try {
    ...
    const targets = rows
      ...
      .map((r) => ({
        ...
        locale: SUPPORTED_LOCALES.includes(r.locale) ? r.locale : DEFAULT_LOCALE,
      }));
    ...
    for (const batch of chunk(targets, 100)) {
      const messages = batch.map(({ token, locale }) => ({
        to: token,
