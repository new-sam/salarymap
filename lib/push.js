...
async function deliver(messages) {
  ...
  const invalid = [];
  const sent = []; // 발송 성공 건 → push_sent 적재용 { category, userId }
  for (const batch of chunk(messages, 100)) {
    // _log는 내부 로깅 전용 필드 — Expo 페이로드에선 제외하고 보낸다.
    const payload = batch.map(({ _log, ...m }) => m);
    let json;
    try {
    ...
    // 응답 ticket: 성공(ok)은 발송 로깅, DeviceNotRegistered는 토큰 정리.
    const tickets = Array.isArray(json?.data) ? json.data : [];
    tickets.forEach((ticket, i) => {
      if (ticket?.status === 'error' && ticket?.details?.error === 'DeviceNotRegistered') {
        invalid.push(batch[i].to);
      } else if (ticket?.status === 'ok' && batch[i]?._log) {
        sent.push(batch[i]._log);
      }
    });
  }

  // 발송수(클릭률 분모) 적재 — 클릭/수신과 같은 events 테이블·meta.platform='app' 규약. 실패해도 토큰 정리는 진행.
  if (sent.length) {
    try {
      await supabaseAdmin.from('events').insert(
        sent.map((lg) => ({
          event: 'push_sent',
          user_id: lg.userId || null,
          meta: { platform: 'app', category: lg.category || null, user_id: lg.userId || null },
        }))
      );
    } catch (e) {
      console.error('[push] push_sent log failed:', e);
    }
  }
...
export async function sendPush(userIds, { title, body, category, data = {} }) {
  try {
    ...
    const { data: rows, error } = await supabaseAdmin
      .from('push_tokens')
      .select('user_id, expo_push_token, locale, prefs, enabled')
      .in('user_id', ids)
      .eq('enabled', true);
    if (error || !rows?.length) return;

    // 클릭 카테고리 정합: data.category로 실어 모바일이 push_click meta.category로 집계하게 한다
    // (그래야 발송수(push_sent)와 같은 카테고리축에서 CTR 비교 가능).
    const payloadData = category ? { ...data, category } : data;

    // 카테고리 opt-out(prefs[category] === false) 제외. 키 없으면 기본 on.
    // 토큰별 locale을 유지해 발송 시 언어를 고른다. _log는 deliver가 push_sent 적재에만 쓰고 Expo로는 안 보낸다.
    const messages = rows
      ...
      .map((r) => {
        ...
        return {
          ...
          data: payloadData,
          sound: 'default',
          _log: { category: category || null, userId: r.user_id },
        };
      });
    ...
    for (const it of list) {
      ...
      for (const { token, locale } of toks) {
        messages.push({
          ...
          data: it.data || {},
          sound: 'default',
          // 클릭 측 pushMeta가 data.category ?? data.type로 카테고리를 잡으므로 동일 규칙으로 발송 로깅.
          _log: { category: it.data?.category ?? it.data?.type ?? null, userId: it.userId },
        });
      }
    }
