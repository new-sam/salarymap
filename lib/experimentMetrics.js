...
export async function computeExperimentMetrics() {
  ...
  for (let i = WINDOW_DAYS - 1; i >= 0; i--) {
    ...
    days[d] = { day: d, signups: 0, submissions: 0, linked: 0, gateClicks: 0, gateLogins: 0, resultGateViews: 0, resultGateClicks: 0, oneTapSignups: 0 }
  }
  const bump = (iso, key) => {
  ...
  for (const e of gateEvents) {
    if (e.event === 'company_gate_click') {
      bump(e.created_at, 'gateClicks')
      if (e.meta?.source === 'result') bump(e.created_at, 'resultGateClicks') // 하드게이트 노출→클릭 분모용
    }
  ...
  const summary = (rows) => ({
    ...
    resultGateClicks: sum(rows, 'resultGateClicks'),
