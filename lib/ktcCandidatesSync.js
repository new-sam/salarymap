...
// 푸시는 보내지 않는다 (일괄 소급 반영이라 화면 표시만). idempotent.
...
export async function syncFyiRejections({ dry = false } = {}) {
  ...
  if (legacy.length) {
    ...
    for (const j of jobs) {
      const code = (String(j.source_id || '').match(/^([A-Z]{2,6}\d{3,4})/) || [])[1]
    ...
    for (let i = 0; i < emails.length; i += 100) {
      const { data, error } = await fyi
        .from('job_applications')
        .select('id, job_id, status, rejected_at, applicant_email')
  ...
  return { ktcRejected: rejected.length, matched: apps.length, updated: targets.length, alreadyDone: apps.length - targets.length, dry }
}
