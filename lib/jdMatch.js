...
export function judge(raw, c, years) {
  ...
  /* 우대사항이 점수에도 들어간다. 자격요건은 통과선이라 통과자끼리는 다 같은 값이 되고,
     그 다음을 가르는 건 우대사항이다 — 기업이 "있으면 좋겠다"고 적어 둔 건 결국
     이 사람과 저 사람 중 누구냐를 정할 때 쓰라고 적은 것이다.
     우대사항을 안 적은 JD 는 만점을 준다. 안 그러면 우대사항을 안 쓴 JD 라는 이유로
     모든 후보의 점수가 통째로 낮아진다(레퍼런스가 등급에서 쓰는 면제와 같은 이유다). */
  const prefRate = preferredTotal ? preferredMet / preferredTotal : 1
  const fit = Math.round(55 * ratio + 0.35 * score + 10 * prefRate)
