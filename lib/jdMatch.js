...
/* 스킬 이름 맞추기 — "React.js" 와 "React", "Node.js" 와 "NodeJS" 를 같은 것으로 본다.
   문자열 포함(hasToken)만 쓰면 JD 가 "React.js" 로 적었을 때 "React" 만 적은 이력서가
   1차에서 통째로 잘린다 — 모델이 볼 기회 자체가 없어지는 자리라 여기가 더 아프다.
   짧은 토큰(Go·C·R)은 정확 일치만 허용한다. 부분일치를 열면 "Go" 로 "Google" 이 걸린다. */
export const normSkill = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9+#]/g, '')
...
export function prefilterScore(p, c) {
  ...
  // 스킬 칸은 정규화 비교를 한 번 더 거친다 — 표기 변형으로 여기서 잘리면 끝이라서다.
  // 키워드(도메인·산업)는 문장 속 단어라 정규화가 오히려 오탐을 만든다: hasToken 만 쓴다.
  const skillNorms = asSkills(p.skills).map(normSkill).filter(Boolean)
  const skillHas = (k) => {
    const n = normSkill(k)
    if (!n) return false
    if (skillNorms.includes(n)) return true
    if (n.length < 3) return false
    return skillNorms.some((s) => s.length >= 3 && (s.includes(n) || n.includes(s)))
  }
  const hitSkills = (list) => (list || []).filter((k) => hasToken(hay, k) || skillHas(k))

  let score = 0
  // 직무가 맞으면 크게 — 같은 스킬을 써도 직무가 다르면 다른 사람이다
  if ((c.positions || []).includes(p.position)) score += 30

  const must = hitSkills(c.must_skills)
  if ((c.must_skills || []).length) score += (must.length / c.must_skills.length) * 40
  score += Math.min(hitSkills(c.nice_skills).length * 4, 16)
