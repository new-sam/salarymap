...
const RULES = [
  ...
  // 디자인 명문 — 명문대 목록이 공대·상경 중심이라 디자인 인재가 구조적으로 걸러지던 것 보완(8/4).
  // strong 은 KPI top-tier 집계엔 안 잡히고 어드민 최우수 인재 기준에만 반영된다.
  { canonical: 'Van Lang University', tier: 'strong',
    test: s => has(s, 'van lang') },
  { canonical: 'University of Architecture (Kiến trúc)', tier: 'strong',
    test: s => s.includes('kien truc') || s.includes('architecture') },
  { canonical: 'University of Fine Arts (Mỹ thuật)', tier: 'strong',
    test: s => s.includes('my thuat') || has(s, 'fine', 'art') },
]
