...
/* QA 세션 — 주소에 ?qa=1 을 한 번 달고 들어오면 그 탭에서는 이벤트를 아예 안 보낸다.
   로그인 전 퍼널 단계(cv_view, cv_file_select …)는 email 이 없어서 서버 쪽 계정 제외로는
   못 걸러내기 때문. sessionStorage 라 탭을 닫으면 풀린다 — 실사용자 브라우저에 눌러붙지 않게. */
function isQaSession() {
  if (typeof window === 'undefined') return false
  try {
    if (new URLSearchParams(window.location.search).get('qa') === '1') sessionStorage.setItem('sm_qa', '1')
    return sessionStorage.getItem('sm_qa') === '1'
  } catch {
    return false
  }
}
