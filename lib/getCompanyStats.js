...
export async function getCompanyStats() {
  ...
  // Compute topPct: what % of ALL individual submissions earn less than this company's median
  // e.g. if 80% of submissions are below Grab's median → Grab is "Top 20%"
  const allSalaries = clean.map(s => s.salary).sort((a, b) => a - b);
  const totalSubs = allSalaries.length;
  return stats.map(s => {
    const below = allSalaries.filter(sal => sal < s.median).length;
    const pctBelow = below / totalSubs; // e.g. 0.82 means 82% earn less
    // topPct = bottom of the top bracket this company is in (5, 10, 15, 20, 25, 30...)
    const rawTopPct = Math.round((1 - pctBelow) * 100);
    // Round to nearest 5, floor at 5
    const topPct = Math.max(5, Math.round(rawTopPct / 5) * 5);
    return { ...s, topPct };
  });
}
