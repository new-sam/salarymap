...
export async function getCompanyStats() {
  ...
  // Compute topPct: company salary rank relative to others
  const medians = stats.map(s => s.median).sort((a, b) => b - a);
  return stats.map(s => {
    const rank = medians.findIndex(m => m <= s.median);
    const topPct = Math.max(1, Math.round((rank / medians.length) * 100)) || 5;
    return { ...s, topPct };
  });
}
