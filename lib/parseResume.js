...
export function preserveUserLanguage(update, profile) {
  const filled = (v) => String(v || '').trim() !== ''
  const out = { ...update }
  if (filled(profile?.english_cert)) delete out.english_cert
  if (filled(profile?.korean_cert)) delete out.korean_cert
  return out
}
...
export async function parseResumeForUser(userId, { touchUpdatedAt = true } = {}) {
  const { data: profile, error: profileErr } = await supabase
    .from('user_profiles').select('id, resume_url, full_name, photo_url, english_cert, korean_cert')
    .eq('id', userId).single()
  ...
  // 사람이 넣은 어학 값은 지키고 나머지만 갱신한다.
  const update = preserveUserLanguage(await parseResumeBuffer(fileBuffer, profile.full_name), profile)
